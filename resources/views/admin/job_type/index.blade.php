                ...
                <div class="portlet light portlet-fit portlet-datatable bordered">
                    <div class="portlet-title">
                        ...
                        <div class="actions">
                            ...
                            <a href="{{ route('create.job.type') }}" class="btn btn-xs btn-success"><i class="glyphicon glyphicon-plus"></i> Add New Job Type</a>
                        </div>
                    </div>
                                ...
                                <table class="table table-striped table-bordered table-hover"  id="jobTypeDatatableAjax">
                                    <thead>
                                        ...
                                        <tr role="row" class="heading">                                            
                                            ...
                                            <th style="min-width:250px;">Job Type</th>
    ...
    $(function () {
        ...
        $('#btnBulkDelete').on('click', function () {
            ...
            if (selectedIds.length === 0) {
                alert('Please select at least one job type to delete.');
                return;
            }

            if (confirm('Are you sure you want to permanently delete ' + selectedIds.length + ' selected job type(s)?')) {
                $('#btnBulkDelete').prop('disabled', true).html('<i class="fa fa-spinner fa-spin"></i> Deleting...');
                $.ajax({
                    url: "{{ route('bulk.delete.job.types') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        ids: selectedIds
                    ...
                    success: function (response) {
                        ...
                        if (response.status === 'ok') {
                            alert(response.count + ' job type(s) deleted successfully!');
                            oTable.draw(false);
                        } else {
                            alert('Failed to delete selected job types: ' + (response.message || 'Unknown error'));
                        }
                    },
                    error: function () {
                        $('#btnBulkDelete').prop('disabled', false).html('<i class="glyphicon glyphicon-trash"></i> Delete Selected (<span id="selectedCount">0</span>)');
