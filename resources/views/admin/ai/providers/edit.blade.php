    ...
    <div class="page-content">
        ...
        <div class="page-bar" style="margin-bottom: 20px;">
            <ul class="page-breadcrumb">
                ...
                <li><span>Edit Provider: {{ $provider->name }}</span></li>
            </ul>
        </div>
        ...
        <div class="portlet light bordered" style="border-radius: 10px; max-width: 850px;">
            <div class="portlet-title">
                <div class="caption font-dark">
                    ...
                    <span class="caption-subject bold uppercase">Edit AI Provider: {{ $provider->name }}</span>
                    @if($provider->is_default)
                        <span class="badge badge-success" style="background-color: #2563EB; font-size: 10px; margin-left: 8px;">PRIMARY ACTIVE</span>
                    @endif
                </div>
                <div class="actions">
            ...
            <div class="portlet-body form">
                <form action="{{ route('admin.ai.providers.update', $provider->id) }}" method="POST" class="form-horizontal">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-body" style="padding: 20px 10px;">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-8">
                                <input type="text" name="name" class="form-control" placeholder="e.g. Primary Gemini Flash, Production OpenAI" value="{{ old('name', $provider->name) }}" required style="border-radius: 6px;">
                            ...
                            <div class="col-md-8">
                                <select name="provider_type" id="provider_type" class="form-control" required style="border-radius: 6px;">
                                    ...
                                    @foreach($supportedProviders as $key => $prov)
                                        <option value="{{ $key }}" {{ old('provider_type', $provider->provider_type) == $key ? 'selected' : '' }}>
                                            {{ $prov['name'] }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-8">
                                <div class="input-group">
                                    ...
                                    <input type="text" name="model" id="model_input" class="form-control" placeholder="e.g. gemini-1.5-flash, gpt-4o-mini" value="{{ old('model', $provider->model) }}" required style="border-radius: 0 6px 6px 0;">
                                </div>
                        ...
                        <!-- API Key -->
                        <div class="form-group">
                            <label class="col-md-3 control-label"><strong>API Key / Token</strong></label>
                            <div class="col-md-8">
                                <div class="input-group">
                                    <input type="password" name="api_key" id="api_key_input" class="form-control" placeholder="Leave blank to keep existing encrypted key" autocomplete="new-password" style="border-radius: 6px 0 0 6px;">
                                ...
                                <div style="margin-top: 6px; font-size: 12px; color: #64748B;">
                                    Current Stored Key: <span style="font-family: monospace; background: #F1F5F9; padding: 2px 6px; border-radius: 4px; color: #334155;">{{ $provider->masked_api_key }}</span>
                                </div>
                                <span class="help-block" style="font-size: 12px; color: #03855c;">
                                    <i class="fa fa-shield"></i> Leave this field empty if you do not wish to change the existing API key.
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-8">
                                <input type="url" name="base_url" class="form-control" placeholder="https://api.openai.com/v1 (Leave blank for default vendor endpoint)" value="{{ old('base_url', $provider->base_url) }}" style="border-radius: 6px;">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-4">
                                <input type="number" name="timeout_sec" class="form-control" value="{{ old('timeout_sec', $provider->timeout_sec) }}" min="5" max="120" style="border-radius: 6px;">
...
var currentModel = @json($provider->model);
    ...
    function updateModels(providerType) {
        ...
        if (providerType && supportedProviders[providerType]) {
            ...
            $.each(models, function(val, label) {
                var isSelected = (val === currentModel) ? 'selected' : '';
