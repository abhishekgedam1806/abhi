...
<div class="form-body">        
    ...
    <div class="form-group {!! APFrmErrHelp::hasError($errors, 'area_name') !!}" id="area_div">
        {!! Form::label('area_name', 'Area / Locality (e.g. Hinjewadi, Dharampeth)', ['class' => 'bold']) !!}
        <input type="text" name="area_name" id="area_name" class="form-control" list="admin_areas_datalist" placeholder="Select or type area / locality..." value="{{ old('area_name', (isset($job) ? $job->area_name : '')) }}">
        <datalist id="admin_areas_datalist"></datalist>
        ...
        if (state_id != '') {
            $.post("{{ route('filter.default.cities.dropdown') }}", {state_id: state_id, city_id: city_id, _method: 'POST', _token: '{{ csrf_token() }}'})
                    .done(function (response) {
                        ...
                        if (activeCityId) loadAdminAreas(activeCityId);
                    });
        }
    }

    function loadAdminAreas(city_id) {
        if (!city_id) {
            $('#admin_areas_datalist').empty();
            return;
        }
        $.ajax({
            ...
            success: function(data) {
                var datalist = $('#admin_areas_datalist');
                datalist.empty();
