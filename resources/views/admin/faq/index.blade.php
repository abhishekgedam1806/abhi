                                ...
                                <table class="table table-striped table-bordered table-hover"  id="faq_datatable_ajax">
                                    <thead>
                                        <tr role="row" class="filter">                  
                                            ...
                                            <td><input type="text" class="form-control" name="faq_question" id="faq_question" autocomplete="off" placeholder="Question"></td>
                                            <td><input type="text" class="form-control" name="faq_answer" id="faq_answer" autocomplete="off" placeholder="Answer"></td>
                                            <td></td>
                                        </tr>
...
<script>
    $(function () {
        ...
        $('#btnBulkDelete').on('click', function () {
            ...
            if (confirm('Are you sure you want to permanently delete ' + selectedIds.length + ' selected FAQ(s)?')) {
                $('#btnBulkDelete').prop('disabled', true).html('<i class="fa fa-spinner fa-spin"></i> Deleting...');
                $.ajax({
                    url: "{{ route('bulk.delete.faqs') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        ids: selectedIds
                    ...
                    success: function (response) {
                        ...
                        if (response.status === 'ok') {
                            alert(response.count + ' FAQ(s) deleted successfully!');
                            oTable.draw(false);
                        } else {
                            alert('Failed to delete selected FAQs: ' + (response.message || 'Unknown error'));
                        }
                    },
                    error: function () {
                        $('#btnBulkDelete').prop('disabled', false).html('<i class="glyphicon glyphicon-trash"></i> Delete Selected (<span id="selectedCount">0</span>)');
