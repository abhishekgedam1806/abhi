    ...
    <div class="page-content">
        <div class="blog-editor-wrap">
            ...
            <div class="blog-breadcrumb">
                ...
                <i class="fa fa-chevron-right"></i>
                <span style="color: #0F172A; font-weight: 700;">Create Blog Post</span>
            </div>
            ...
            <div class="blog-header-row">
                <h1 class="blog-page-title">
                    <span class="title-icon"><i class="fa fa-pencil-square-o"></i></span>
                    Create New Blog Post
                </h1>
                <div style="display: flex; gap: 8px;">
                    <a href="{{ route('blog') }}" class="btn btn-default" style="border-radius: 8px; font-weight: 600;">Cancel</a>
                </div>
            </div>

            <form id="formCreateBlog" method="POST" action="{{ url('admin/blog/create') }}" enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="action_type" id="action_type" value="publish">
                        ...
                        <div class="card-box">
                            <div class="form-group mb-3">
                                ...
                                <select name="lang" id="blog_lang" class="form-control-custom">
                                    ...
                                    <option value="{{ $code }}" {{ $code === 'en' ? 'selected' : '' }}>{{ $langName }} ({{ $code }})</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group mb-3">
                                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px;">
                                    ...
                                    <button type="button" class="btn-ai-sparkle" onclick="requestAiSeo('meta_title')">
                                        <i class="fa fa-magic"></i> ✨ AI Suggest Title
                                    </button>
                                </div>
                                <input type="text" name="title" id="blog_title" class="form-control-custom" placeholder="e.g. How to Find High Paying IT Jobs in Mumbai in 2026" required autofocus value="{{ old('title') }}" style="font-size: 15px; font-weight: 700;">
                            ...
                            <div class="form-group mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" name="slug" id="blog_slug" class="form-control" placeholder="how-to-find-it-jobs" required value="{{ old('slug') }}" style="border-color: #E2E8F0; border-radius: 0 8px 8px 0; font-size: 13px;">
                                </div>
                                @if(isset($errors) && $errors->has('slug'))
                                <span class="text-danger" style="font-size: 12px;">{{ $errors->first('slug') }}</span>
                                @endif
                            </div>

                            <div class="form-group mb-3">
                                <label class="form-label-custom">Blog Article Content <span class="text-danger">*</span></label>
                                <textarea name="content" id="description" class="form-control" rows="18" placeholder="Write full article here...">{!! old('content') !!}</textarea>
                                @if(isset($errors) && $errors->has('content'))
                                <span class="text-danger" style="font-size: 12px;">{{ $errors->first('content') }}</span>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="card-box" style="padding: 16px 22px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px;">
                            <div>
                                <label style="display: flex; align-items: center; gap: 8px; margin: 0; font-weight: 700; font-size: 13px; color: #475569; cursor: pointer;">
                                    <input type="checkbox" name="featured" value="1" style="width: 18px; height: 18px;"> Mark as Featured Post
                                </label>
                            </div>
                            <div style="display: flex; gap: 10px;">
                                ...
                                <button type="submit" onclick="$('#action_type').val('publish')" class="btn btn-primary" style="background: #2563EB; border-color: #2563EB; border-radius: 8px; font-weight: 700; padding: 9px 24px; box-shadow: 0 2px 6px rgba(37,99,235,0.25);">
                                    <i class="fa fa-send"></i> Publish Post
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-5 col-12">
                        ...
                        <div class="card-box">
                            ...
                            <div class="seo-score-box">
                                ...
                                <div>
                                    ...
                                    <div style="font-size: 11.5px; color: #64748B;" id="seoScoreSummary">Enter title & content to evaluate SEO</div>
                                </div>
                            </div>
                            ...
                            <div class="google-preview-card">
                                <div class="google-preview-url">
                                    <span>{{ url('/blog') }}/</span><strong id="previewSlug">post-slug</strong>
                                </div>
                                <div class="google-preview-title" id="previewTitle">Enter a title for this blog post...</div>
                                <div class="google-preview-desc" id="previewDesc">Enter a meta description to see how this career article will appear in Google search results.</div>
                            </div>
                            ...
                            <div style="background: #F8FAFC; border: 1.5px solid #E2E8F0; border-radius: 10px; padding: 14px 16px; margin-bottom: 18px;">
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" name="focus_keyword" id="focus_keyword" class="form-control-custom" placeholder="e.g. IT Jobs in Mumbai" value="{{ old('focus_keyword') }}">
                                </div>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" name="meta_title" id="meta_title" class="form-control-custom" placeholder="Search engine title..." value="{{ old('meta_title') }}">
                                </div>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <textarea name="meta_descriptions" id="meta_descriptions" class="form-control-custom" rows="3" placeholder="Engaging snippet for Google results...">{{ old('meta_descriptions') }}</textarea>
                                    <div style="margin-top: 6px; text-align: right;">
                                        <button type="button" class="btn-ai-sparkle" onclick="requestAiSeo('meta_description')">
                                            <i class="fa fa-magic"></i> ✨ AI Suggest Description
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div class="form-group mb-0">
                                    ...
                                    <input type="text" name="meta_keywords" id="meta_keywords" class="form-control-custom" placeholder="e.g. jobs, career, mumbai hiring" value="{{ old('meta_keywords') }}">
                                </div>
                            </div>
                            ...
                            <div class="seo-accordion-item">
                                ...
                                <div class="seo-accordion-body" id="body-seoReadability">
                                    <div class="seo-check-item" id="check-title-len"><i class="fa fa-times-circle"></i> <span>Title length optimal (under 60 chars)</span></div>
                                    <div class="seo-check-item" id="check-desc-len"><i class="fa fa-times-circle"></i> <span>Meta description optimal (120-155 chars)</span></div>
                            ...
                            <div class="seo-accordion-item">
                                ...
                                <div class="seo-accordion-body" id="body-seoSocial">
                                    <div class="form-group mb-2">
                                        ...
                                        <input type="text" name="og_title" id="og_title" class="form-control-custom" placeholder="Leave empty to use Meta Title" value="{{ old('og_title') }}">
                                    </div>
                                    <div class="form-group mb-2">
                                        <label class="form-label-custom" style="font-size: 12px;">OG / Social Description</label>
                                        <textarea name="og_description" id="og_description" class="form-control-custom" rows="2" placeholder="Leave empty to use Meta Description">{{ old('og_description') }}</textarea>
                                    </div>
                                    <div class="form-group mb-0">
                            ...
                            <div class="seo-accordion-item">
                                ...
                                <div class="seo-accordion-body" id="body-seoTechnical">
                                    <div class="form-group mb-2">
                                        ...
                                        <input type="text" name="canonical_url" id="canonical_url" class="form-control-custom" placeholder="Leave empty for auto: {{ url('/blog') }}/slug" value="{{ old('canonical_url') }}">
                                    </div>
                                    <div class="row">
                                    ...
                                    <div style="font-size: 11px; color: #03855c; background: #ECFDF5; padding: 6px 10px; border-radius: 6px; margin-top: 4px;">
                                        <i class="fa fa-check"></i> BlogPosting Schema JSON-LD auto-generated on publish.
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card-box">
                            ...
                            <div class="form-group mb-3">
                                ...
                                <div style="max-height: 180px; overflow-y: auto; border: 1.5px solid #E2E8F0; border-radius: 8px; padding: 10px 12px; background: #F8FAFC;">
                                    ...
                                    <label style="display: flex; align-items: center; gap: 8px; font-size: 13px; font-weight: 500; color: #334155; margin-bottom: 6px; cursor: pointer;">
                                        <input type="checkbox" name="cate_id[]" value="{{ $cate->id }}" style="cursor: pointer;">
                                        {!! $cate->heading !!}
                                    </label>
                                    @endforeach
                                </div>
                            ...
                            <div class="form-group mb-0">
                                ...
                                <input type="file" name="image" id="featured_image_input" class="form-control-custom" accept="image/*" onchange="previewFeaturedImage(this)">
                        ...
                        <div class="card-box">
                            ...
                            <div class="form-group mb-2">
                                ...
                                <input type="text" name="author_name" class="form-control-custom" placeholder="e.g. Abhishek Sharma" value="{{ old('author_name', 'Abhishek Sharma') }}">
                            </div>
                            <div class="form-group mb-2">
                                <label class="form-label-custom" style="font-size: 12.5px;">Professional Title / Designation</label>
                                <input type="text" name="author_title" class="form-control-custom" placeholder="e.g. Career Consultant & Lead Editor" value="{{ old('author_title', 'Career Consultant & Lead Editor') }}">
                            </div>
                            <div class="form-group mb-0">
                                <label class="form-label-custom" style="font-size: 12.5px;">Author Bio</label>
                                <textarea name="author_bio" class="form-control-custom" rows="2" placeholder="Short bio displayed at the end of the post...">{{ old('author_bio', 'Passionate career counselor and recruitment specialist sharing expert job search strategies, interview preparation techniques, and hiring insights across India.') }}</textarea>
                            </div>
                        </div>
...
// Automatic slug generation on title input
$('#blog_title').on('input', function() {
    var title = $(this).val();
    if (!$('#blog_slug').data('touched')) {
        var slug = title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
        $('#blog_slug').val(slug);
    }
    if (!$('#meta_title').data('touched')) {
...
$('#meta_title, #meta_descriptions, #focus_keyword').on('input', function() {
    $(this).data('touched', true);
    calculateDeterministicSeo();
});
...
function calculateDeterministicSeo() {
    ...
    var hasImg = $('#featured_image_input').val() !== '' || $('#imagePreviewContainer').is(':visible');
...
$('#btnAcceptAiSuggestion').on('click', function() {
    ...
    if (pendingAiField === 'meta_title') {
        $('#meta_title').val(pendingAiText).data('touched', true);
    } else if (pendingAiField === 'meta_description') {
        $('#meta_descriptions').val(pendingAiText).data('touched', true);
    } else if (pendingAiField === 'focus_keyword') {
        $('#focus_keyword').val(pendingAiText).data('touched', true);
    }
