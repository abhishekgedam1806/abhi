...
<div style="padding: 12px 20px; border-bottom: 1.5px solid #E2E8F0; background: #FFFFFF; display: flex; align-items: center; justify-content: space-between; flex-shrink: 0;">
    <div style="display: flex; align-items: center; gap: 12px;">
        ...
        <div>
            <h2 style="font-size: 14.5px; font-weight: 800; color: #0F172A; margin: 0;">{{ $company->name }}</h2>
            <span style="font-size: 11.5px; color: #03855c; font-weight: 600; display: flex; align-items: center; gap: 4px;">
                <i class="fa fa-circle" style="font-size: 7px;"></i> {{__('Active Recruiter')}}
            </span>
        </div>
    </div>
    <a href="{{ route('company.detail', $company->slug) }}" target="_blank" style="background: #EFF6FF; color: #2563EB; font-size: 12px; font-weight: 700; padding: 6px 12px; border-radius: 8px; text-decoration: none; display: inline-flex; align-items: center; gap: 6px;">
        <i class="fa fa-external-link"></i> {{__('View Profile')}}
    </a>
</div>

<!-- Chat Message Bubbles Stream -->
<ul class="chat-messages-container message{{$company->id}}" style="flex: 1; min-height: 0; overflow-y: auto; padding: 20px; display: flex; flex-direction: column; gap: 14px; list-style: none; margin: 0;">
            ...
            @else
                ...
                <li class="chat-bubble-sent tab{{$company->id}}">
                    <div style="width: 32px; height: 32px; border-radius: 8px; overflow: hidden; border: 1px solid #E2E8F0; background: #FFFFFF; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                        ...
                        @else
                            <span style="font-weight: 800; color: #2563EB; font-size: 12px;">{{ strtoupper(substr($seeker->first_name ?: 'U', 0, 1)) }}</span>
                        @endif
                    </div>
                    <div class="bubble-content">
                        <div>{{ $msg->message }}</div>
...
<div class="chat-input-bar">
    <form class="form-chat-send" style="margin: 0; display: flex; align-items: center; gap: 10px;">
        @csrf
        <input type="hidden" name="company_id" value="{{$company->id}}">
        <div style="flex: 1; position: relative;">
            <input type="text" class="form-control" name="message" id="chat_message_input" placeholder="{{__('Type your message here...')}}" autocomplete="off" style="width: 100%; height: 44px; border: 1.5px solid #CBD5E1; border-radius: 10px; padding: 8px 14px; font-size: 13.5px; background: #F8FAFC; outline: none; box-shadow: none;">
        </div>
...
$(document).ready(function() {
    if ($(".form-chat-send").length > 0) {
        $(".form-chat-send").validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    ...
                    type: "POST",
                    data: $('.form-chat-send').serialize(),
                    success: function(res) {
                        $(".form-chat-send").trigger("reset");
                        $('.chat-messages-container').html(res);
