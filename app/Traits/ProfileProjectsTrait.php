...
trait ProfileProjectsTrait
{
    ...
    public function showFrontProfileProjects(Request $request, $user_id)
    {
        ...
        if (isset($user) && count($user->profileProjects)) {
            foreach ($user->profileProjects as $project) {
                $date_end = ($project->is_on_going == 1) ? '<span class="badge-default-cv" style="font-size:11px; padding:2px 8px;">Ongoing</span>' : ($project->date_end ? $project->date_end->format('M Y') : '');
                $date_start = $project->date_start ? $project->date_start->format('M Y') : '';
                $image = ImgUploader::get_image("project_images/thumb/$project->image", 0, 0, '/admin_assets/no-image.png', $project->name);

                $html .= '<div class="col-md-6 col-12 mb-3" id="project_' . $project->id . '">
                    <div class="profile-timeline-card h-100" style="padding:16px;">
                        <div style="display:flex; align-items:flex-start; gap:12px;">
                            <div style="width:56px; height:56px; border-radius:8px; overflow:hidden; flex-shrink:0; background:#F8FAFC; border:1px solid #E2E8F0;">
                                ' . $image . '
                            </div>
                            <div style="flex-grow:1;">
                                <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:6px;">
                                    <h5 style="font-size:15px; font-weight:700; color:#0F172A; margin:0 0 2px 0;">' . e($project->name) . '</h5>
                                    <div class="profile-actions-wrap">
                                        <a href="javascript:void(0);" onclick="showProfileProjectEditModal(' . $project->id . ');" class="btn-action-edit" style="padding:3px 8px; font-size:11px;"><i class="fa fa-pencil"></i></a>
                                        <a href="javascript:void(0);" onclick="delete_profile_project(' . $project->id . ');" class="btn-action-delete" style="padding:3px 8px; font-size:11px;"><i class="fa fa-trash"></i></a>
                                    </div>
                                </div>
                                <div style="font-size:12px; color:#64748B; margin-bottom:4px;"><i class="fa fa-calendar"></i> ' . $date_start . ' - ' . $date_end . '</div>
                                <p style="font-size:13px; color:#475569; margin:0; line-height:1.4;">' . Str::limit($project->description, 70, '...') . '</p>
                            </div>
                        </div>
                    </div>
                </div>';
            ...
            $html .= '<div class="col-12"><div class="profile-empty-state">
                <div class="profile-empty-state-text"><i class="fa fa-folder-open-o"></i> No portfolio projects added yet. Showcase your past projects and work to impress employers.</div>
            </div></div>';
        }
        $html .= '</div>';
        echo $html;
        ...
        if (isset($user) && count($user->profileProjects)) {
            ...
            foreach ($user->profileProjects as $project) {
                if ($project->is_on_going == 1) {
                    ...
                    $date_end = $project->date_end ? $project->date_end->format('d M, Y') : '';
                }
                ...
                <div class="imgbox">' . $image . '
                  <div class="itemHover">
                    <div class="zoombox"><a href="' . $project->url . '" title="' . e($project->name) . '" class="item-zoom fancybox-effects-a"><i class="fa fa-search" aria-hidden="true"></i></a> </div>
                    <div class="infoItem">
                      <div class="itemtitle">
                        <h5>' . e($project->name) . '</h5>
                        <p>' . ($project->date_start ? $project->date_start->format('d M, Y') : '') . ' - ' . $date_end . '<br />
						  ' . Str::limit($project->description, 25, '...') . '</p>
                      </div>
                    </div>
                  </div>
            ...
            $html .= '<p class="text-muted" style="padding: 10px 0; font-size: 13.5px;"><i class="fa fa-folder-open-o text-muted"></i> ' . __('No portfolio projects added yet.') . '</p>';
        }
