...
trait CountryStateCity
{
    ...
    {
        $country = $this->relationLoaded('country') ? $this->country : $this->country()->lang()->first();
        if (null === $country) {
    ...
    {
        $state = $this->relationLoaded('state') ? $this->state : $this->state()->lang()->first();
        if (null === $state) {
    ...
    {
        $city = $this->relationLoaded('city') ? $this->city : $this->city()->lang()->first();
        if (null === $city) {
    ...
    public function getGoogleMapEmbedUrl()
    {
        // 1. If custom map embed code or URL is provided
        if (!empty($this->map)) {
            if (preg_match('/src=["\']([^"\']+)["\']/', $this->map, $match)) {
                return $match[1];
            }
            if (filter_var($this->map, FILTER_VALIDATE_URL)) {
                return $this->map;
            }
        }

        // 2. Auto-generate from office address + city + state + country
        $parts = [];
        ...
        $fullAddress = implode(', ', array_unique($parts));
        if (empty($fullAddress)) {
            $fullAddress = 'India';
        }

        return 'https://maps.google.com/maps?q=' . urlencode($fullAddress) . '&t=&z=14&ie=UTF8&iwloc=&output=embed';
    }
