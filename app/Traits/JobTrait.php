...

{
...

    {
        ...
        $job->area_id = $request->input('area_id');

        $job->area_name = $request->input('area_name');

        if (!empty($job->area_id) && empty($job->area_name)) {
            $areaObj = \App\Area::find($job->area_id);
            if ($areaObj) $job->area_name = $areaObj->area_name;
        }
...

    {
		...
		if((bool)config('company.is_company_package_active')){
...

			{
				...
				flash(__('Please subscribe to a package or upgrade your plan to post new jobs.'))->error();

				return \Redirect::route('pricing', ['tab' => 'employers']);
