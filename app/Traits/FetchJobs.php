...
trait FetchJobs
{
    ...
    public function fetchJobs($search = '', $job_titles = array(), $company_ids = array(), $industry_ids = array(), $job_skill_ids = array(), $functional_area_ids = array(), $country_ids = array(), $state_ids = array(), $city_ids = array(), $is_freelance = -1, $career_level_ids = array(), $job_type_ids = array(), $job_shift_ids = array(), $gender_ids = array(), $degree_level_ids = array(), $job_experience_ids = array(), $salary_from = 0, $salary_to = 0, $salary_currency = '', $is_featured = -1, $orderBy = 'id', $limit = 10)
    {
        $filters = [
            ...
            'state_id' => $state_ids,
            'city_id' => $city_ids,
            'career_level_id' => $career_level_ids,
            'job_type_id' => $job_type_ids,
            'job_shift_id' => $job_shift_ids,
            'gender_id' => $gender_ids,
            'degree_level_id' => $degree_level_ids,
            'job_experience_id' => $job_experience_ids,
            'functional_area_id' => $functional_area_ids,
        ...
        if (request()->has('lat') && request()->has('lon')) {
            $userCoords = ['lat' => (float)request()->get('lat'), 'lon' => (float)request()->get('lon')];
        }

        $result = \App\Services\JobRecommendationEngine::searchAndRankJobs($search, $filters, $user, $userCoords, $limit);
        return $result['paginator'];
    }

    public function fetchFacetSummary($search = '', $job_titles = array(), $company_ids = array(), $industry_ids = array(), $job_skill_ids = array(), $functional_area_ids = array(), $country_ids = array(), $state_ids = array(), $city_ids = array(), $is_freelance = -1, $career_level_ids = array(), $job_type_ids = array(), $job_shift_ids = array(), $gender_ids = array(), $degree_level_ids = array(), $job_experience_ids = array(), $salary_from = 0, $salary_to = 0, $salary_currency = '', $is_featured = -1)
    {
        $fields = [
            'jobs.id',
            'jobs.title',
        ...
        return [
            'jobTitlesArray' => array_values(array_filter(array_unique($results->pluck('title')->toArray()))),
            'jobIdsArray' => array_values(array_filter(array_unique($results->pluck('id')->toArray()))),
            'countryIdsArray' => array_values(array_filter(array_unique($results->pluck('country_id')->toArray()))),
            'stateIdsArray' => array_values(array_filter(array_unique($results->pluck('state_id')->toArray()))),
            'cityIdsArray' => array_values(array_filter(array_unique($results->pluck('city_id')->toArray()))),
            'companyIdsArray' => array_values(array_filter(array_unique($results->pluck('company_id')->toArray()))),
            'functionalAreaIdsArray' => array_values(array_filter(array_unique($results->pluck('functional_area_id')->toArray()))),
            'careerLevelIdsArray' => array_values(array_filter(array_unique($results->pluck('career_level_id')->toArray()))),
            'jobTypeIdsArray' => array_values(array_filter(array_unique($results->pluck('job_type_id')->toArray()))),
            'jobShiftIdsArray' => array_values(array_filter(array_unique($results->pluck('job_shift_id')->toArray()))),
            'genderIdsArray' => array_values(array_filter(array_unique($results->pluck('gender_id')->toArray()))),
            'degreeLevelIdsArray' => array_values(array_filter(array_unique($results->pluck('degree_level_id')->toArray()))),
            'jobExperienceIdsArray' => array_values(array_filter(array_unique($results->pluck('job_experience_id')->toArray()))),
        ];
    }
    ...
    public function createQuery($query, $search = '', $job_titles = array(), $company_ids = array(), $industry_ids = array(), $job_skill_ids = array(), $functional_area_ids = array(), $country_ids = array(), $state_ids = array(), $city_ids = array(), $is_freelance = -1, $career_level_ids = array(), $job_type_ids = array(), $job_shift_ids = array(), $gender_ids = array(), $degree_level_ids = array(), $job_experience_ids = array(), $salary_from = 0, $salary_to = 0, $salary_currency = '', $is_featured = -1)
    {
        ...
        if (!empty($area_ids) && is_array($area_ids)) {
            $query->whereIn('jobs.area_id', $area_ids);
        } elseif (!empty($area_ids)) {
            $query->where('jobs.area_id', $area_ids);
        }
        if ($is_freelance == 1 || (is_array($is_freelance) && in_array(1, $is_freelance)) || $is_freelance === '1') {
