...
class CompanyController extends Controller
{
    ...
    public function index()
    {
        ...
        $recentJobs = $company->jobs()->with(['city', 'jobType'])->orderBy('id', 'desc')->take(4)->get();
        $totalJobsCount = $company->jobs()->count();
        $openJobsCount = $company->countOpenJobs();
        $followersCount = $company->countFollowers();
        $messagesCount = $company->countCompanyMessages();

        $jobIds = $company->jobs()->pluck('id')->toArray();
        $totalApplicantsCount = \App\JobApply::whereIn('job_id', $jobIds)->count();
        $shortlistedCount = \App\JobApply::whereIn('job_id', $jobIds)->where(function($q) use ($company) {
            $q->where('status', 'shortlisted')
              ->orWhereIn('job_id', function($sub) use ($company) {
                  $sub->select('job_id')->from('favourite_applicants')->where('company_id', $company->id);
              });
        })->count();

        // Recent applicants
        $recentApplicants = \App\JobApply::whereIn('job_id', $jobIds)
            ->with(['user', 'job'])
            ->orderBy('id', 'desc')
            ->take(4)
            ->get();

        return view('company_home', compact(
            'company',
            'recentJobs',
            'totalJobsCount',
            'openJobsCount',
    ...
    public function updateCompanyProfile(CompanyFrontFormRequest $request)
    {
        ...
        $company->hr_name = $request->input('hr_name');
        $company->whatsapp_number = $request->input('whatsapp_number');
        $company->allow_phone_contact = $request->input('allow_phone_contact', 0);
        $company->allow_whatsapp_contact = $request->input('allow_whatsapp_contact', 0);
		
        $company->slug = Str::slug($company->name, '-') . '-' . $company->id;
        $company->update();
		// Newsletter disabled for local development

        flash(__('Company profile updated successfully!'))->success();
    ...
    public function updateApplicationStatus(Request $request)
    {
        ...
        $allowed = ['applied', 'shortlisted', 'hired', 'rejected'];
        ...
        if ($status === 'shortlisted' || $status === 'hired') {
            FavouriteApplicant::firstOrCreate([
                'user_id' => $job_application->user_id,
                'job_id' => $job_application->job_id,
                'company_id' => $company_id
            ]);
        } elseif ($status === 'rejected' || $status === 'applied') {
            FavouriteApplicant::where('user_id', $job_application->user_id)
        ...
        return response()->json([
            ...
            'message' => 'Candidate status updated to ' . ($statusLabels[$status] ?? ucfirst($status)) . ' successfully!'
        ]);
    }
