...
class UserController extends Controller
{
    ...
    public function updateUserImage(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $userName = $user->first_name ? $user->first_name . ' ' . $user->last_name : 'user-' . $user->id;
            $fileName = ImgUploader::UploadImage('user_images', $image, $userName, 300, 300, true);
            $user->image = $fileName;
            $user->update();

            $imageSrc = asset('user_images/' . $fileName);
