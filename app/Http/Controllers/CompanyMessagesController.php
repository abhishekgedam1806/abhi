...
class CompanyMessagesController extends Controller
{
    ...
    function submit_message(Request $request)
    {
        ...
        \App\AppNotification::sendNotification(
            ...
            'user',
            $request->seeker_id,
            __('New Message from :company', ['company' => $company->name]),
            __(':company sent you a message: ":msg"', ['company' => $company->name, 'msg' => \Illuminate\Support\Str::limit($request->message, 80)]),
            route('my.messages'),
            'fa-comments',
            '#2563EB'
        );
