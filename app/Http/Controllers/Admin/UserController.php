    ...
    public function fetchUsersData(Request $request)
    {
        ...
        return Datatables::of($users)
                        ...
                        ->addColumn('checkbox', function ($users) {
                            return '<input type="checkbox" class="row-checkbox" value="' . $users->id . '" style="cursor:pointer;width:17px;height:17px;" />';
                        })
				...
				<div class="btn-group" style="white-space:nowrap;">
					...
					<ul class="dropdown-menu dropdown-menu-right" style="min-width:170px;padding:6px 0;border-radius:8px;box-shadow:0 10px 25px -5px rgba(0,0,0,0.12);border:1px solid #e2e8f0;z-index:10050;">
						<li>
							<a href="' . route('edit.user', ['id' => $users->id]) . '" style="padding:8px 14px;display:flex;align-items:center;gap:8px;font-size:12.5px;color:#334155;"><i class="fa fa-pencil text-primary" style="width:16px;"></i> Edit</a>
						</li>						
						<li>
							<a href="javascript:void(0);" onclick="delete_user(' . $users->id . ');" style="padding:8px 14px;display:flex;align-items:center;gap:8px;font-size:12.5px;color:#dc2626;"><i class="fa fa-trash-o text-danger" style="width:16px;"></i> Delete</a>
						</li>
                        <li class="divider" style="margin:4px 0;"></li>
						<li>
						<a href="javascript:void(0);" onClick="' . $active_href . '" id="onclick_active_' . $users->id . '" style="padding:8px 14px;display:flex;align-items:center;gap:8px;font-size:12.5px;color:#334155;"><i class="fa fa-' . $active_icon . '" style="width:16px;"></i> ' . $active_txt . '</a>
						</li>
						<li>
						<a href="javascript:void(0);" onClick="' . $verified_href . '" id="onclick_verified_' . $users->id . '" style="padding:8px 14px;display:flex;align-items:center;gap:8px;font-size:12.5px;color:#334155;"><i class="fa fa-' . $verified_icon . '" style="width:16px;"></i> ' . $verified_txt . '</a>
						</li>																																							
					</ul>
				</div>';
                        })
    ...
    public function bulkDeleteUsers(Request $request)
    {
        ...
        if (!empty($ids) && is_array($ids)) {
            try {
                ...
                DB::table('job_apply')->whereIn('user_id', $ids)->delete();
                DB::table('favourites_job')->whereIn('user_id', $ids)->delete();
                DB::table('favourites_company')->whereIn('user_id', $ids)->delete();
