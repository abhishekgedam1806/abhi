...
class SliderController extends Controller
{
    ...
    public function storeSlider(SliderFormRequest $request)
    {
		...
		if ($request->hasFile('slider_image')) {
			...
			$fileName = ImgUploader::UploadImage('slider_images', $request->file('slider_image'), $image_name, 1600, 640);
    ...
    public function fetchSlidersData(Request $request)
    {
        ...
        return Datatables::of($sliders)
                        ...
                        ->addColumn('checkbox', function ($sliders) {
                            return '<input type="checkbox" class="row-checkbox" value="' . $sliders->id . '" style="cursor:pointer;width:17px;height:17px;" />';
                        })
				...
				<div class="btn-group" style="white-space:nowrap;">
					...
					<ul class="dropdown-menu dropdown-menu-right" style="min-width:150px;padding:6px 0;border-radius:8px;box-shadow:0 10px 25px -5px rgba(0,0,0,0.12);border:1px solid #e2e8f0;z-index:10050;">
						<li>
							<a href="' . route('edit.slider', ['id' => $sliders->id]) . '" style="padding:8px 14px;display:flex;align-items:center;gap:8px;font-size:12.5px;color:#334155;"><i class="fa fa-pencil text-primary" style="width:16px;"></i> Edit</a>
						</li>						
						<li>
							<a href="javascript:void(0);" onclick="deleteSlider(' . $sliders->id . ', ' . $sliders->is_default . ');" style="padding:8px 14px;display:flex;align-items:center;gap:8px;font-size:12.5px;color:#dc2626;"><i class="fa fa-trash-o text-danger" style="width:16px;"></i> Delete</a>
						</li>
                        <li class="divider" style="margin:4px 0;"></li>
						<li>
						<a href="javascript:void(0);" onClick="' . $activeHref . '" id="onclickActive' . $sliders->id . '" style="padding:8px 14px;display:flex;align-items:center;gap:8px;font-size:12.5px;color:#334155;"><i class="fa fa-' . $activeIcon . '" style="width:16px;"></i> ' . $activeTxt . '</a>
						</li>																																		
					</ul>
				</div>';
                        })
                        ->rawColumns(['checkbox', 'action', 'slider_heading'])
    ...
    public function bulkDeleteSliders(Request $request)
    {
        ...
        if (!empty($ids) && is_array($ids)) {
            try {
                ...
                $slider_ids = Slider::whereIn('id', $ids)->pluck('slider_id')->filter()->toArray();
                if (!empty($slider_ids)) {
                    Slider::whereIn('slider_id', $slider_ids)->delete();
                }
