...
class AdminController extends Controller
{
    ...
    public function bulkDeleteAdminUsers(Request $request)
    {
        ...
        if (!empty($ids) && is_array($ids)) {
            try {
                ...
                if (empty($ids)) {
                    return response()->json(['status' => 'error', 'message' => 'Cannot delete currently logged in admin account.']);
                }
    ...
    public function fetchAdminUsersData()
    {
        ...
        return Datatables::of($users)
                        ->addColumn('checkbox', function ($user) {
                            return '<input type="checkbox" class="row-checkbox" value="' . $user->id . '" style="cursor:pointer;width:17px;height:17px;" />';
                        })
                        ->addColumn('action', function ($user) {
                            return '<div style="display:inline-flex;gap:4px;"><a href="' . route('edit.admin.user', ['id' => $user->id]) . '" class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-edit"></i> Edit</a><a href="javascript:void(0);" onclick="delete_user(' . $user->id . ');" class="btn btn-xs btn-danger"><i class="glyphicon glyphicon-remove"></i> Delete</a></div>';
                        })
