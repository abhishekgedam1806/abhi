...
class JobSkillController extends Controller
{
    ...
    public function indexJobSkills()
    {
        ...
        return view('admin.job_skill.index')
    ...
    public function storeJobSkill(JobSkillFormRequest $request)
    {
        ...
        $jobSkill->functional_area_id = $request->input('functional_area_id');
    ...
    {
        $jobSkills = JobSkill::select(['job_skills.id', 'job_skills.functional_area_id', 'job_skills.job_skill', 'job_skills.is_active', 'job_skills.lang', 'job_skills.is_default', 'job_skills.created_at', 'job_skills.updated_at'])->sorted();
        return Datatables::of($jobSkills)
                        ->filter(function ($query) use ($request) {
                            ...
                            if ($request->has('functional_area_id') && !empty($request->get('functional_area_id'))) {
                                $query->where('job_skills.functional_area_id', '=', $request->get('functional_area_id'));
                            }
                        ...
                        ->addColumn('department', function ($jobSkills) {
                            $fa = $jobSkills->functionalArea;
                            return $fa ? '<span class="badge" style="background:#03855c; color:#fff; font-weight:600;">' . e($fa->functional_area) . '</span>' : '<span class="text-muted">General</span>';
                        })
                        ->addColumn('job_skill', function ($jobSkills) {
                            ...
                            return '<span dir="' . $direction . '" style="font-weight:600; color:#1E293B;">' . $jobSkill . '</span>';
                        })
				...
				<div class="btn-group">
					...
					<ul class="dropdown-menu">
						...
						<li>
							<a href="javascript:void(0);" onclick="' . $activeHref . '" class=""><i class="fa fa-' . $activeIcon . '" aria-hidden="true"></i>' . $activeTxt . '</a>
						</li>
						<li>
                        ...
                        ->rawColumns(['job_skill', 'department', 'action'])
