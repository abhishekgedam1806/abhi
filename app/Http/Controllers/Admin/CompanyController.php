    ...
    public function fetchCompaniesData(Request $request)
    {
        ...
        return Datatables::of($companies)
                        ...
                        ->addColumn('checkbox', function ($companies) {
                            return '<input type="checkbox" class="row-checkbox" value="' . $companies->id . '" style="cursor:pointer;width:17px;height:17px;" />';
                        })
                        ->addColumn('is_active', function ($companies) {
                            return ((bool) $companies->is_active) ? '<span class="label label-sm label-success">Yes</span>' : '<span class="label label-sm label-danger">No</span>';
                        })
                        ->addColumn('is_featured', function ($companies) {
                            return ((bool) $companies->is_featured) ? '<span class="label label-sm label-info">Yes</span>' : '<span class="label label-sm label-default">No</span>';
                        })
				...
				<div class="btn-group" style="white-space:nowrap;">
					...
					<ul class="dropdown-menu dropdown-menu-right" style="min-width:170px;padding:6px 0;border-radius:8px;box-shadow:0 10px 25px -5px rgba(0,0,0,0.12);border:1px solid #e2e8f0;z-index:10050;">
						<li>
							<a href="' . route('list.jobs', ['company_id' => $companies->id]) . '" target="_blank" style="padding:8px 14px;display:flex;align-items:center;gap:8px;font-size:12.5px;color:#334155;"><i class="fa fa-list text-info" style="width:16px;"></i> List Jobs</a>
						</li>
						<li>
							<a href="' . route('edit.company', ['id' => $companies->id]) . '" style="padding:8px 14px;display:flex;align-items:center;gap:8px;font-size:12.5px;color:#334155;"><i class="fa fa-pencil text-primary" style="width:16px;"></i> Edit Company</a>
						</li>						
						<li>
							<a href="javascript:void(0);" onclick="deleteCompany(' . $companies->id . ');" style="padding:8px 14px;display:flex;align-items:center;gap:8px;font-size:12.5px;color:#dc2626;"><i class="fa fa-trash-o text-danger" style="width:16px;"></i> Delete</a>
						</li>
                        <li class="divider" style="margin:4px 0;"></li>
						<li><a href="javascript:void(0);" onClick="' . $activeHref . '" id="onclickActive' . $companies->id . '" style="padding:8px 14px;display:flex;align-items:center;gap:8px;font-size:12.5px;color:#334155;"><i class="fa fa-' . $activeIcon . '" style="width:16px;"></i> ' . $activeTxt . '</a></li>
						<li><a href="javascript:void(0);" onClick="' . $featuredHref . '" id="onclickFeatured' . $companies->id . '" style="padding:8px 14px;display:flex;align-items:center;gap:8px;font-size:12.5px;color:#334155;"><i class="fa fa-' . $featuredIcon . '" style="width:16px;"></i> ' . $featuredTxt . '</a></li>
					</ul>
				</div>';
                        })
                        ->rawColumns(['checkbox', 'action', 'is_active', 'is_featured'])
        ...
        if (!empty($ids) && is_array($ids)) {
            try {
                foreach ($ids as $company_id) {
                    $this->deleteCompanyLogo($company_id);
                }
                $job_ids = DB::table('jobs')->whereIn('company_id', $ids)->pluck('id')->toArray();
                if (!empty($job_ids)) {
                ...
                DB::table('favourites_company')->whereIn('company_id', $ids)->delete();
