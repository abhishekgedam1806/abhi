...
class FunctionalAreaController extends Controller
{
    ...
    public function fetchFunctionalAreasData(Request $request)
    {
        ...
        return Datatables::of($functionalAreas)
                        ...
                        ->addColumn('checkbox', function ($functionalAreas) {
                            return '<input type="checkbox" class="row-checkbox" value="' . $functionalAreas->id . '" style="cursor:pointer;width:17px;height:17px;" />';
                        })
				...
				<div class="btn-group" style="white-space:nowrap;">
					...
					<ul class="dropdown-menu dropdown-menu-right" style="min-width:150px;padding:6px 0;border-radius:8px;box-shadow:0 10px 25px -5px rgba(0,0,0,0.12);border:1px solid #e2e8f0;z-index:10050;">
						<li>
							<a href="' . route('edit.functional.area', ['id' => $functionalAreas->id]) . '" style="padding:8px 14px;display:flex;align-items:center;gap:8px;font-size:12.5px;color:#334155;"><i class="fa fa-pencil text-primary" style="width:16px;"></i> Edit</a>
						</li>						
						<li>
							<a href="javascript:void(0);" onclick="deleteFunctionalArea(' . $functionalAreas->id . ', ' . $functionalAreas->is_default . ');" style="padding:8px 14px;display:flex;align-items:center;gap:8px;font-size:12.5px;color:#dc2626;"><i class="fa fa-trash-o text-danger" style="width:16px;"></i> Delete</a>
						</li>
                        <li class="divider" style="margin:4px 0;"></li>
						<li>
						<a href="javascript:void(0);" onClick="' . $activeHref . '" id="onclickActive' . $functionalAreas->id . '" style="padding:8px 14px;display:flex;align-items:center;gap:8px;font-size:12.5px;color:#334155;"><i class="fa fa-' . $activeIcon . '" style="width:16px;"></i> ' . $activeTxt . '</a>
						</li>																																		
					</ul>
				</div>';
                        })
                        ->rawColumns(['checkbox', 'functional_area', 'action'])
    ...
    public function bulkDeleteFunctionalAreas(Request $request)
    {
        ...
        if (!empty($ids) && is_array($ids)) {
            try {
                $fa_ids = FunctionalArea::whereIn('id', $ids)->pluck('functional_area_id')->filter()->toArray();
                if (!empty($fa_ids)) {
                    FunctionalArea::whereIn('functional_area_id', $fa_ids)->delete();
                }
                FunctionalArea::whereIn('id', $ids)->delete();
