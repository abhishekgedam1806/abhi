...
class User extends Authenticatable
{
    ...
    /**
     * Check if user is Job Seeker / Candidate / Employee
     */
    public function isJobSeeker()
    {
        return empty($this->user_type) || $this->user_type === 'jobseeker';
    }
    ...
    public function getName()
    {
        ...
        if (empty($html) && !empty($this->name)) {
            $html = trim($this->name);
        }

        if (empty($html) && !empty($this->email)) {
            $html = explode('@', $this->email)[0];
        }
