...
class ProfileSkill extends Model
{
    ...
    {
        return $this->belongsTo('App\JobSkill', 'job_skill_id', 'job_skill_id')->where('job_skills.lang', 'en');
    }
    ...
    public function getJobSkill($field = '')
    {
        ...
        if (null === $jobSkill) {
            $jobSkill = \App\JobSkill::where('id', $this->job_skill_id)->first();
        }
        if (null === $jobSkill) {
            $jobSkill = \App\JobSkill::where('job_skill_id', $this->job_skill_id)->first();
        }
        if (null !== $jobSkill) {
            if (empty($field))
                return $jobSkill;
